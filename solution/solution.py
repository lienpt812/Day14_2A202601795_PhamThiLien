...
def _clamp_score(score: float) -> float:
    """Keep heuristic scores within the normalized metric range."""
...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        if relevant_seen == 0:
            return 0.0
        return _clamp_score(precision_sum / relevant_seen)
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        prompt = (
            "Score the answer against the rubric. Return JSON with criterion "
            "names mapped to scores from 0 to 1.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
        ...
        scores_source = parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
        scores: dict[str, float] = {}
        for criterion in rubric:
            try:
                score = float(scores_source[criterion])
            except (KeyError, TypeError, ValueError):
                score = 0.5
            scores[criterion] = _clamp_score(score)
        ...
        for item in scores_batch:
            ...
            for key, value in scores.items():
                try:
                    ...
                    continue
                all_scores.append(numeric_value)
                if key == "first":
                    first_scores.append(numeric_value)
                elif "first" in scores:
                    other_scores.append(numeric_value)

        avg_score = sum(all_scores) / len(all_scores) if all_scores else 0.0
        positional_bias = False
        if first_scores and other_scores:
            positional_bias = (sum(first_scores) / len(first_scores)) > (
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def avg_attr(results: list[EvalResult], attr: str) -> float:
            if not results:
                return 0.0
            return sum(getattr(result, attr) for result in results) / len(results)

        metrics = ("faithfulness", "relevance", "completeness")
        new_avgs = {metric: avg_attr(new_results, metric) for metric in metrics}
        baseline_avgs = {
            metric: avg_attr(baseline_results, metric) for metric in metrics
        }
        regressions = [
            metric
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if len(sorted_scores) > 1 and sorted_scores[1][1] - lowest_score < 0.05:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            rows.append(
                "| F{failure_id:03d} | {failure_type} | {root_cause} | {suggestion} | Open |".format(
                    failure_id=index,
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": "Implement a grounding check that rejects claims not supported by retrieved context",
            "irrelevant": "Tighten the prompt so the answer must directly address the user question",
            "incomplete": "Improve retrieval coverage and ask the generator to include all required answer points",
            "off_topic": "Add intent classification and route unsupported requests before generation",
            "refusal": "Review guardrail rules to reduce unnecessary refusals on in-scope questions",
            "unknown": "Inspect failed examples manually and add targeted golden cases for the pattern",
        }

        ordered_types = sorted(categories, key=lambda key: categories[key], reverse=True)
        suggestions = [
        ...
        if any("retrieval" in cause for cause in root_causes):
            suggestions.append("Increase chunk quality or rerank retrieved chunks to put supporting evidence first")
        if any("prompt" in cause for cause in root_causes):
            suggestions.append("Add few-shot examples that show concise answers aligned to the question")
        if any("missing key information" in cause for cause in root_causes):
            suggestions.append("Add completeness criteria to the generation prompt and benchmark rubric")
        ...
        fallback_suggestions = [
            "Review the lowest-scoring cases and add regression tests for each repeated failure mode",
            "Calibrate metric thresholds against human-reviewed examples before using them as a deployment gate",
            "Track failure categories over time to verify that fixes reduce repeated errors",
        ]
